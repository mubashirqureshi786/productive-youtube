    ...
    <div className="flex items-start justify-between py-3 group">
      ...
      >
        <input
          ...
          onChange={(e) => {
            console.log(
              `ToggleSwitch: Toggle ${id} changed to`,
              e.target.checked
            );
...
const PopupApp: React.FC = () => {
  ...
  console.log("PopupApp: Current settings state:", settings);

  // Load settings from Chrome storage on mount and listen for changes
  useEffect(() => {
    console.log("PopupApp: Mounting - loading settings from storage");

    // Initial load
    if (typeof chrome !== "undefined" && chrome.storage) {
      const keys = Object.keys(defaultSettings);
      chrome.storage.local.get(keys, (result) => {
        console.log("PopupApp: Loaded settings from storage:", result);
        ...
        console.log("PopupApp: Setting initial state:", loadedSettings);
      ...
      const handleStorageChange = (
        ...
        if (areaName === "local") {
          console.log("PopupApp: Storage change detected:", changes);
          setSettings((prevSettings) => {
            ...
            console.log("PopupApp: Updated settings state:", updatedSettings);
  ...
  const handleToggle = (key: keyof Settings, value: boolean) => {
    console.log(`PopupApp: handleToggle called - ${key} to ${value}`);
    console.log(`PopupApp: Current settings before update:`, settings);

    const newSettings = { ...settings, [key]: value };
    console.log("PopupApp: New settings being set locally:", newSettings);
    setSettings(newSettings);

    if (typeof chrome !== "undefined" && chrome.storage) {
      console.log(`PopupApp: Saving ${key} = ${value} to Chrome storage`);
      chrome.storage.local.set({ [key]: value }, () => {
        console.log(`PopupApp: ✅ Successfully saved ${key} = ${value}`);
        // Verify it was saved
        chrome.storage.local.get([key], (result) => {
          console.log(`PopupApp: ✅ Verified ${key} in storage:`, result[key]);
        });
      });
    } else {
      console.warn("PopupApp: Chrome storage not available!");
    }
  };
